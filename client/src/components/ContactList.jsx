...
const ContactList = ({ onSelectContact }) => {
  const { allContacts } = useContext(UserContext);
  ...
  const handleChange = (event) => {
    ...
    setSelectedContact(null); // Reset selected contact when searching
  };
    ...
    <div className={`w-64 h-screen ${theme === 'light' ? 'bg-sky-300 text-gray-800' : 'bg-gradient-to-b from-gray-700 to-gray-900 text-white'} p-4 shadow-lg`}>
      ...
      <div className="relative mb-6">
        ...
        <span className="absolute right-3 top-1/2 transform -translate-y-1/2">
          <AiOutlineSearch className={`h-5 w-5 ${theme === 'light' ? 'text-gray-400' : 'text-gray-300'}`} />
        </span>
      </div>
