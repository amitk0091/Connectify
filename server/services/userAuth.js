...
const loginUser = async (req,res)=>{
        ...
        try {
            ...
            if (match) {
                ...
                res.status(200).json({ message: 'Password matches!' , token});  // http success
            } else {
...
const addUser = async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            return res.status(409).json({ message: 'Email already in use' });  // http conflicting
        }
        ...
        res.status(201).json({ "message": "User registered successfully", token}); // http created 
